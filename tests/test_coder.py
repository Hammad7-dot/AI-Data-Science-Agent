...
def test_classification_basic_saves_model_to_disk(tmp_path):
    ...
    # Every export must include preprocessing for prediction on raw CSV columns.
    import pandas as pd
    raw = pd.read_csv(DATASET_PATH).drop(columns="churn").head()
    assert len(loaded.predict(raw)) == len(raw)
